...
### Todo is a website designed to inspire you to be more productive. Whether your a professional juggling work tasks or a student managing assignments, todo will help you stay on track with all the things you need to get done.
...
### I followed the Agile methology throughout, it helped me to stay on track and motivating me to get stories done one by one.
...
While planning this project, I envisioned to have both tasks and habits as the main features. However, given the limited timeframe, the inclusion of habits was not possible.

The wireframe in difference from the finnished product featured a third navigation bar that would allow users to toggle between tasks and habits. The goal was to create an engaging layout where navigation 3 would adapt to wheter habits or tasks were selected in Navigation2, interchanging the options for sorting and ordering based on if tasks or habits are selected for display. It would also have a button that would change from Create New Task to Add New Habit depending on selection in navigation2.

Although I had to set the habits feature aside for the time being, the database models for habits remain in the backend. My intention is to revisit the original wireframe concept, and reintroducing the habits component into the project. Due to the time constraints, I introduced the 'notes' feature, which though unplanned complements the projects functionality.
...
As mentioned in the Wireframe section, the original idea behind this project was to have tasks and habits. However, given the limited timeframe, and drawing from my experience with learning new programming languages, libraries, and frameworks within tight schedules, I realized that prioritising habits might not be duable, and therefore, no user stories were made in the beginning while planning the project. As it turned out, my intuition was correct, including habits was not an option in the end.
...
Instead of habits, I introduced the 'notes' feature as an alternative, which although it was unplanned, integrates well into the project. In the future, I will return to the original wireframe concept, and use the 'notes' component into the user's profile, prompting them to write motivational messages. These messages will be displayed between Navigation 1 and Navigation 2 in the wireframe I made.

What I've learned from past experiences is that while planning is crucial, you must also recognize when it is necessary to let go of certain ideas due to time constraints. I encountered numerous bugs, which provided valuable lessons on tokens, authentication, date convertions and more. Unfortunately, these issues consumed a lot of time, preventing me from implementing the habits feature.

There are several areas in this project that can be improved, including defensive programming for task deletion, being able to access the account when forgetting a password, refining date conversions, and fixing repetitive styling, and adding some exciting features to the profile page. There are also aspects that needs to be fixed in this project, for example occasionally it encounters an unauthorized access bug, requiring a hard refresh.

As mentioned above, there is a few issues to fix, and there are several desirable additions to add.
Thus the project is not perfect, I can happily say that I've gatherd a lot more knowledge throughout this very "bugable" experience that I would not have learned if everything went smoothly.

My plan after submission of the project involves to firsly fix the issues mentioned with the unathorization bug harcore refresh, add defensive proframming to task deletion, fix repetetive styling, further modularization and enhancing component reusability. And then adding features like habits, an improved user profile, and even user-to-user interactions for task assignment.
...
Secondly, I had myself in focus. What did I want to personally achieve with this project? My main goal was to gain an understanding of the React architecture, working with APIs, and Django Rest Framework. In the end, I created full CRUD functionality for tasks, and I also had time to add create, read, and deletion functionality for notes. I learned about React and its components and gained insight into advanced JavaScript concepts along the way. I was also introduced to serializers, http requests and much more.

In conclusion, with the final product, I can say that I learned more than I can express, but I know there is so much more to learn, and I am only at the beginning of my journey. However, in the role of a developer, you are never finished; there is always more to learn and room for improvement. Can this project improve? Yes, there is a lot of room, which is great. This project is the perfect choice to keep building on, and little by little, the code will be cleaner, and the website will be greater.
...
- **/signin** (SignInForm.js) Allows user to sign in.

- **/signup** (SignUpForm.js) Allows user to sign up.
...
**TasksAll**
As seen underneath in "usage", the `TasksAll` aka "/" homepage, serves as the central hub for the website. As the parent component, most API calls are made from here, including fetching,deleting, and marking tasks as completed. However, modularization has been thought of and NavTask.js and TaskListItem.js are direct childs of TasksAll inheriting the neccessary props for the reason of seperation of concerns.
...
**Purpose:** It lets users to effortlessly find tasks that match their criteria.
...
**Purpose:** To notify users that their search did not found any matching task titles or descriptions.
...
**Description:** NotAuthenticated.js is a user-friendly welcome message encouraging users to log in or to create an account.
...
### Note.js
**Description:** Note.js handles the retrieval, creation, and deletion of user notes. It renders alongside tasks, providing a quick way write down notes.
...
- `priority`: A option field to select by "High", "Medium", "Low".
...
### The application `profile` has one model that represent the profile of the user.
**`profile`**
...
Unfortunatly links to the profiles uploading the images I used does not work.
...
The logo I used were uploaded by clelia-clelia
